...
const commands: Command[] = [
	{
		...
		icon: <SearchIcon size={16} className="text-[var(--muted)]" />,
	},
	{
		name: "/create-component",
		description: "Create React component with TypeScript & tests",
		agents: ["Claude", "OpenCode", "Gemini"],
		icon: <Component className="h-4 w-4 text-[var(--muted)]" />,
	},
	{
		name: "/fix-tests",
		description: "Diagnose and fix failing tests",
		agents: ["Claude", "OpenCode", "Gemini"],
		icon: <TestTube className="h-4 w-4 text-[var(--muted)]" />,
	},
	{
		name: "/commit",
		description: "Create conventional commit message",
		agents: ["Claude", "OpenCode", "Gemini"],
		icon: <GitCommitIcon size={16} className="text-[var(--muted)]" />,
	},
	{
		name: "/add-shadcn",
		description: "Add shadcn/ui component to project",
		agents: ["Claude", "OpenCode", "Gemini"],
		icon: <PaletteIcon size={16} className="text-[var(--muted)]" />,
	},
	{
		name: "/a11y-audit",
		description: "Audit accessibility issues in code",
		agents: ["Claude", "OpenCode", "Gemini"],
		icon: <AccessibilityIcon size={16} className="text-[var(--muted)]" />,
	},
	{
		name: "/create-issue",
		description: "Create GitHub issue (bug/feature)",
		agents: ["Claude"],
		icon: <CirclePlus className="h-4 w-4 text-[var(--muted)]" />,
	},
	{
		...
		icon: <Image className="h-4 w-4 text-[var(--muted)]" />,
	},
];
			...
			<div className="relative mx-auto max-w-6xl px-6">
				...
				<div className="grid gap-4 md:grid-cols-2">
					{commands.map((cmd) => (
						<div
							key={cmd.name}
							className="card-hover flex items-start gap-4 rounded-lg border border-[var(--border)] bg-[var(--card)]/80 backdrop-blur-sm p-4"
							tabIndex={0}
							role="article"
							aria-label={`${cmd.name} command`}
						>
							<div className="flex shrink-0 items-center gap-3 rounded-md bg-[var(--secondary)]/80 px-3 py-2">
								{cmd.icon}
							...
							<div className="flex-1">
								...
								<div className="mt-2 flex flex-wrap gap-1">
									{cmd.agents.map((agent) => (
										<span
											key={agent}
											className="rounded-full bg-[var(--secondary)]/80 px-2 py-0.5 text-xs text-[var(--muted)] font-mono"
										>
											{agent}
										</span>
									))}
