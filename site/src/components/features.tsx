	...
	const workflows = [
		{
			...
			icon: <SearchIcon size={24} className="text-(--muted)" />,
		},
		{
			name: "component-create",
			description: "Create components with tests",
			icon: <Component className="h-6 w-6 text-(--muted)" />,
		},
		{
			name: "test-coverage",
			description: "Analyze and improve coverage",
			icon: <TestTube className="h-6 w-6 text-(--muted)" />,
		},
		{
			name: "deploy-preview",
			description: "Setup preview deployments",
			icon: <RocketIcon size={24} className="text-(--muted)" />,
		},
	];
